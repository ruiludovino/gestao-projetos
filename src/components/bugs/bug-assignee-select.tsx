...
import { updateBugAssigneeAction } from "@/actions/bugs";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function BugAssigneeSelect({
  bugId,
  value,
  members,
  disabled,
  ...
  function handleChange(next: string | null) {
    ...
    startTransition(async () => {
      try {
        await updateBugAssigneeAction(bugId, next === UNASSIGNED ? null : next);
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "Erro ao atribuir bug.");
      }
    });
  }
